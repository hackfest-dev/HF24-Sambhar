...
class dt_regressor_sambhar:
    ...
    def predictions(self, max_leaf_nodes, user_input):
        ...
        # X_gpu = DMatrix(X, nthread=-1) 
        model = XGBRegressor(n_estimators = 2, max_depth = 2 ,sampling_method = 'uniform',learning_rate = 1, objective='reg:squarederror')
        # use device = 'gpu' and sampling_method = gradient_based for better results
        model.fit(X, self.y)
